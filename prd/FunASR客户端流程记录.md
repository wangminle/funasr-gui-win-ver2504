...
- **中间结果**：
  ```json
  {
    ...
    "chunk_size": [5, 10, 5], 
    "chunk_interval": 10, 
    "audio_fs": 16000,  // 采样率，从文件中读取或使用默认值
    "wav_name": "demo", 
    "wav_format": "pcm", 
    "is_speaking": true, 
    "hotwords": "", 
...
3.  **`wav_name` 问题:** 服务端日志记录的 `wav_name` 是 `"demo"`，而不是客户端日志中显示的实际文件名（如 `tv-report-1.wav`）。这强烈暗示客户端在发送初始配置JSON时，`wav_name` 字段没有被正确设置或被服务端忽略/覆盖了。
