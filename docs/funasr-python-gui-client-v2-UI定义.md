...
**版本**: 2.4 (新增热词文件支持、StatusManager状态管理)  
**最后更新日期:** 2025-11-03
...
*   **默认窗口大小**:
    *   所有平台: 840x720（统一尺寸，确保状态栏在所有平台默认可见）
*   **状态栏可见性**: 窗口尺寸已优化为840x720，确保所有UI要素包括底部状态栏在首次启动时即完整可见，无需手动调整窗口大小。
...
*   **布局**: `pack(padx=10, pady=5, fill=tk.X)`
    ...
    *   **选择热词文件 按钮 (`ttk.Button`)** **（2025-10-23新增）**
        *   **控件变量名**: `self.select_hotword_button`
        *   **文本**: "选择热词文件" / "Select Hotword File"
        *   **位置**: `grid(row=1, column=0, padx=5, pady=2, sticky=tk.W)`
        *   **操作**: 点击后调用 `self.select_hotword_file` 方法，弹出文件选择对话框。
        *   **对话框设置**: `filetypes = (("文本文件", "*.txt"), ("所有文件", "*.*"))`
        *   **效果**: 选择文件后，热词文件路径显示框更新。
        *   **Tooltip**: "热词格式: 每行一个热词，可选权重。例如: 热词 30"（中文）/ "Hotword format: one per line, optional weight. Example: hotword 30"（英文）
    *   **热词文件路径 显示框 (`ttk.Entry`)** **（2025-10-23新增）**
        *   **控件变量名**: `self.hotword_path_entry`
        *   **关联变量**: `self.hotword_path_var` (tk.StringVar)
        *   **大小**: `width=40`, 水平可扩展
        *   **状态**: 只读 (`state='readonly'`)
        *   **位置**: `grid(row=1, column=1, padx=5, pady=2, sticky=tk.EW)`
        *   **效果**: 显示选择的热词文件路径。
    *   **清除热词 按钮 (`ttk.Button`)** **（2025-10-23新增）**
        *   **控件变量名**: `self.clear_hotword_button`
        *   **文本**: "清除热词" / "Clear Hotword"
        *   **位置**: `grid(row=1, column=2, padx=5, pady=2, sticky=tk.W)`
        *   **操作**: 点击后调用 `self.clear_hotword_file` 方法，清空热词文件路径。
        *   **映射**: 如果有热词文件路径，传递 `--hotword <路径>` 参数给识别脚本。
...
*   **效果**: 通过更新 `self.status_var.set(...)` 来显示当前操作的简短状态。

### 8.2 StatusManager类 **（2025-10-23新增）**

**核心职责：** 统一管理GUI状态显示，提供清晰的视觉反馈。

**状态类型与颜色：**
*   **SUCCESS（成功）**: 绿色 (#008000) - 操作成功完成
*   **INFO（信息）**: 蓝色 (#0000FF) - 正常流程信息
*   **WARNING（警告）**: 橙色 (#FF8C00) - 需要注意的情况
*   **ERROR（错误）**: 红色 (#FF0000) - 错误或失败
*   **PROCESSING（处理中）**: 灰色 (#666666) - 正在进行的操作
...
**临时状态自动恢复：**
*   短暂操作提示（如"文件已选择"）显示3秒后自动恢复到"准备就绪"
*   使用`self.after(3000, ...)`实现异步状态恢复
