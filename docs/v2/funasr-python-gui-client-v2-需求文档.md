...
*   ✅ **支持热词文件（已完成-2025-10-23）:** 在 GUI 添加了"选择热词文件"按钮、"清除热词"按钮和路径显示框，将路径传递给 `--hotword` 参数，并提供Tooltip格式说明（中英文）。配置文件支持热词路径保存和加载。
*   ✅ **配置输出目录（已完成）:** 已实现固定输出目录到 `dev/output`，并提供了"打开结果目录"按钮。未来可考虑允许用户自定义输出目录。
*   ✅ **增强错误处理（已完成）:** 对常见的错误进行捕获，并在 GUI 中以友好的方式告知用户。实现了音频文件时长获取失败的错误处理和兜底机制。
*   ✅ **添加"取消"功能（v2.3已简化移除）:** v2.3代码对齐时移除了过度复杂的取消识别功能，提高了稳定性。
*   **多文件处理体验:** 当用户选择 `.scp` 文件时，可以在日志或状态栏显示正在处理哪个文件，以及总体进度。
*   ✅ **配置持久化（已完成）:** 将用户上次使用的服务器 IP、端口、高级选项（ITN, SSL）、热词文件路径保存到配置文件，下次启动时自动加载。配置文件保存在 `dev/config/config.json`。
*   **UI/UX 细节打磨:** 调整控件间距、对齐，为按钮和选项添加 Tooltips 提示，使界面更美观易用。（热词Tooltip已完成-2025-10-23，其他控件待补充）
*   **打包与分发:** 使用 Nuitka 或 PyInstaller 将应用打包成可执行文件，方便非开发用户使用，并确保包含所有依赖。
*   ✅ **速度测试功能（已完成）:** 已实现用于测试服务器上传速度和转写速度的功能，包括专用的测试按钮、状态显示和结果展示区域。使用demo目录中的音视频文件自动执行测试，无需用户手动选择文件。
*   ✅ **国际化支持（已完成）:** 添加中英文界面切换功能，满足不同语言背景用户的需求，所有UI文本使用资源文件管理，便于扩展添加更多语言，语言设置保存到配置文件，下次启动时自动加载上次的语言选择。

**总结优先级（更新至2025-11-03）:**

✅ **已完成（优先实施）:**
- **(3) 结果与日志分离** - 已实现选项卡式UI分离
- **(4) 生成日志文件** - 已实现日志轮转和分类
- **(1) 状态栏信息优化** - 已实现StatusManager类，5种状态类型，8个识别阶段（2025-10-23）
- **(5) 增强错误处理** - 已实现友好的错误提示和兜底策略
- **(5) 热词支持** - 已完成GUI热词文件选择功能（2025-10-23）
- **配置输出目录** - 已实现固定到`dev/output`
- **国际化支持** - 已完成中英文界面切换

**待实施（按P0/P1/P2优先级）:**
- **P0:** 连接测试抽象、日志清理策略
- **P1:** Online/2Pass模式支持、Tooltips补充、.scp批量优化
- **P2:** 打包分发、配置增强、性能优化
