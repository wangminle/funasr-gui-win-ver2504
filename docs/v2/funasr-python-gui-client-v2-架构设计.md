...
│   │   ├── connection_tester.py       # 连接测试模块 (2025-10-24新增)
│   │   └── requirements.txt           # 依赖管理
│   └── tools/                  # 工具脚本
│       └── run_lints.py        # Lint与类型检查脚本
├── dev/                        # 开发运行时目录
│   ├── config/                 # 配置文件
│   │   ├── config.json         # 主配置文件
│   │   ├── flake8.ini          # Flake8配置
...
### 3.2 目录职责说明
- **src/**: 源代码存放，所有代码使用中文注释
- **dev/**: 开发运行时目录（配置、日志、输出）
- **dev/config/**: 配置文件目录
- **dev/logs/**: 日志文件目录
- **dev/output/**: 识别结果输出目录（从v2.3开始统一使用）
- **docs/v2/**: V2版本项目文档，markdown格式
- **docs/technical-review/**: 技术评审相关文档
- **tests/**: 测试脚本和测试报告
- **ref/**: 参考资料，不允许修改
...
### 4.2.5 连接测试模块 (connection_tester.py) - 2025-10-24新增

#### 模块职责
封装WebSocket连接测试逻辑，统一错误处理，提供友好的测试结果。

#### 主要类和枚举

**ErrorType枚举：** 错误类型分类
- `NETWORK`: 网络错误（无法连接、连接拒绝等）
- `PROTOCOL`: 协议错误（消息格式错误、服务器关闭连接等）
- `TIMEOUT`: 超时错误
- `SSL`: SSL相关错误
- `UNKNOWN`: 未知错误

**ConnectionTestResult数据类：** 结构化测试结果
```python
@dataclass
...
**ConnectionTester类：** 连接测试器
- 延迟导入websockets（支持无依赖环境）
- 可配置超时时间
- 可配置初始化消息模板
- 统一的错误解析和用户友好消息生成

#### 核心方法
- `test_connection(host, port, use_ssl)`: 执行连接测试
- `set_init_message(message)`: 设置初始化消息模板
- `set_timeout(timeout)`: 设置超时时间

#### 便捷函数
```python
async def test_connection(host, port, use_ssl=False, timeout=10, init_message=None):
    """便捷函数：测试WebSocket连接"""
```
