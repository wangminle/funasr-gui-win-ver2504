...
sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../src/python-gui-client")
    ),
)
...
class TestConnectionTester(unittest.TestCase):
    ...
    def test_05_user_friendly_messages(self):
        ...
        msg = self.tester._get_user_friendly_message(ErrorType.SSL, ssl.SSLError())
    ...
    async def _async_test_08_connection_refused(self):
        """异步步骤：连接被拒绝场景（实际测试）"""
        print("\n测试8: 连接被拒绝场景")

        # 测试连接到不存在的端口（实际会被拒绝）
        result = await asyncio.wait_for(
            self.tester.test_connection("127.0.0.1", 65535, False),
            timeout=self.tester.timeout + 1,
        )
    ...
    def test_08_connection_refused(self):
        """测试8: 连接被拒绝场景（实际测试）

        说明：
        - unittest.TestCase 不原生支持 async def 测试方法；
        - 为了兼容 pytest/unittest 的执行方式，这里使用 asyncio.run 调用异步步骤，
          避免出现“coroutine was never awaited”的警告，同时确保测试真实执行。
        """
        asyncio.run(self._async_test_08_connection_refused())

    async def _async_test_09_invalid_host(self):
        """异步步骤：无效主机场景"""
        print("\n测试9: 无效主机场景")

        # 测试无效的主机名
        result = await asyncio.wait_for(
            self.tester.test_connection("invalid.host.test", 10095, False),
            timeout=self.tester.timeout + 1,
        )
    ...
    def test_09_invalid_host(self):
        """测试9: 无效主机场景（同步包装）"""
        asyncio.run(self._async_test_09_invalid_host())
...
def run_sync_tests():
    ...
    # 添加同步测试
    suite.addTests(
        loader.loadTestsFromName("__main__.TestConnectionTester.test_01_initialization")
    )
    suite.addTests(
        loader.loadTestsFromName("__main__.TestConnectionTester.test_02_build_uri")
    )
    suite.addTests(
        loader.loadTestsFromName("__main__.TestConnectionTester.test_03_ssl_context")
    )
    suite.addTests(
        loader.loadTestsFromName(
            "__main__.TestConnectionTester.test_04_parse_error_types"
        )
    )
    suite.addTests(
        loader.loadTestsFromName(
            "__main__.TestConnectionTester.test_05_user_friendly_messages"
        )
    )
    suite.addTests(
        loader.loadTestsFromName(
            "__main__.TestConnectionTester.test_06_connection_test_result"
        )
    )
    suite.addTests(
        loader.loadTestsFromName("__main__.TestConnectionTester.test_07_setters")
    )
    suite.addTests(
        loader.loadTestsFromName(
            "__main__.TestConnectionTester.test_10_convenience_function"
        )
    )
...
async def run_async_tests():
    ...
    try:
        print("\n执行异步测试...")
        await test_case._async_test_08_connection_refused()
    ...
    try:
        await test_case._async_test_09_invalid_host()
