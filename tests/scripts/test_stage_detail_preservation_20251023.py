...
class TestStageDetailPreservation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with (
            ...
            self.lang_manager = funasr_gui_client_v3.LanguageManager()

            # 创建 StatusManager 实例
            self.status_manager = funasr_gui_client_v3.StatusManager(
                self.mock_status_var, self.mock_status_bar, self.lang_manager
            )
    ...
    def test_stage_preparing_with_eta(self):
        ...
        # 模拟带 ETA 的准备阶段
        eta_detail = "预计2分30秒"
        self.status_manager.set_stage(self.status_manager.STAGE_PREPARING, eta_detail)
        ...
        self.assertIn(
            eta_detail, status_text, f"状态文本应包含 ETA 信息 '{eta_detail}'"
        )

        # 验证包含准备任务的提示
        self.assertIn("准备识别任务", status_text, "状态文本应包含'准备识别任务'")
    ...
    def test_stage_connecting_with_server_info(self):
        ...
        self.status_manager.set_stage(
            self.status_manager.STAGE_CONNECTING, server_detail
        )
        ...
        print(f"  状态文本: {status_text}")
        self.assertIn(
            server_detail, status_text, f"状态文本应包含服务器信息 '{server_detail}'"
        )

        # 验证包含连接服务器的提示
        self.assertIn("连接服务器", status_text, "状态文本应包含'连接服务器'")
    ...
    def test_stage_reading_file_with_filename(self):
        ...
        print(f"  状态文本: {status_text}")
        self.assertIn(filename, status_text, f"状态文本应包含文件名 '{filename}'")
    ...
    def test_stage_uploading_with_progress(self):
        ...
        progress = "45%"
        self.status_manager.set_stage(self.status_manager.STAGE_UPLOADING, progress)

        call_args = self.mock_status_var.set.call_args
        status_text = call_args[0][0] if call_args and call_args[0] else ""

        print(f"  状态文本: {status_text}")
        self.assertIn(progress, status_text, f"状态文本应包含进度 '{progress}'")
    ...
    def test_stage_completed_with_result(self):
        ...
        result_info = " (1分30秒)"
        self.status_manager.set_stage(self.status_manager.STAGE_COMPLETED, result_info)
        ...
        self.assertIn(
            result_info, status_text, f"状态文本应包含结果信息 '{result_info}'"
        )
